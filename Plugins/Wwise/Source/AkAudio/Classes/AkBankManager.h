...
public:
	...
	const TSet<UAkAudioBank*> GetLoadedBankList() const
	{
		...
		for (const TStrongObjectPtr<UAkAudioBank>& Bank : LoadedBanks)
		{
			Banks.Add(Bank.Get());
		}
		return Banks;
...
private:
	void AddBankInternal(TStrongObjectPtr<UAkAudioBank> Bank);
	void RemoveBankInternal(TStrongObjectPtr<UAkAudioBank> Bank);

private:
	static FAkBankManager* Instance;
	TSet<TStrongObjectPtr<UAkAudioBank>> LoadedBanks;
