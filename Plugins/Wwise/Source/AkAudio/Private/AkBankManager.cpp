...
void FAkBankManager::AddLoadedBank(UAkAudioBank* Bank)
{
	...
	if (IsInGameThread())
	{
		AddBankInternal(TStrongObjectPtr<UAkAudioBank>(Bank));
	}
...
void FAkBankManager::RemoveLoadedBank(UAkAudioBank* Bank)
{
	...
	if (IsInGameThread())
	{
		RemoveBankInternal(TStrongObjectPtr<UAkAudioBank>(Bank));
	}
...
void FAkBankManager::AddBankInternal(TStrongObjectPtr<UAkAudioBank> Bank)
{
	FScopeLock AutoLock(&BankLock);
...
void FAkBankManager::RemoveBankInternal(TStrongObjectPtr<UAkAudioBank> Bank)
{
	FScopeLock AutoLock(&BankLock);
